...
void printGyro()
{
  ...
  Serial.print((dof.calcGyro(dof.gx))-1, 2);
  Serial.print(", ");
  Serial.print((dof.calcGyro(dof.gy))-1.75, 2);
  Serial.print(", ");
  Serial.println((dof.calcGyro(dof.gz)) + 7.4, 2);
...
void printOrientation(float &pitch, float &roll)
{
  //float *pitch, *roll;
  //float pitch, roll;
  short accData[3];
  ...
  accData[0] = dof.calcAccel(dof.ax)/1.07;
  accData[1] = dof.calcAccel(dof.ay)/.99;
  accData[2] = dof.calcAccel(dof.az)/.95;

  gyrData[0] = dof.calcGyro(dof.gx)-1;
  gyrData[1] = dof.calcGyro(dof.gy)-1.75;
  gyrData[2] = dof.calcGyro(dof.gz);
